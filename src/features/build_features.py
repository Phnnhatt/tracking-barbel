...
# Load data
# --------------------------------------------------------------
df = pd.read_pickle("../../data/interim/02_outliers-removed_chauvenets.pkl")

prediction_columns = list(df.columns[:6])
...
pc_values = PCA.determine_pc_explained_variance(df_pca, prediction_columns)

plt.figure(figsize=(10, 10))
plt.plot(range(1, len(prediction_columns) + 1), pc_values)
plt.xlabel("principal component number")
plt.ylabel("explained variance")
plt.show()

df_pca = PCA.apply_pca(df_pca, prediction_columns, 3)
...
prediction_columns = prediction_columns + ["acc_y", "gyr_y"]
...
subset[["gyr_y", "acc_y_temp_mean_ws_5", "acc_y_temp_std_ws_5"]].plot()
...
for s in df_freq["set"].unique():
    ...
    subset = FreqAbs.abstract_frequency(subset, prediction_columns, ws, fs)
